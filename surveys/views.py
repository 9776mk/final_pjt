...
def index(request):
    ...
    careers_dict = {
        '1년 미만': 0, '1년 이상': 0, '2년 이상': 0, '3년 이상': 0,
        '4년 이상': 0, '5년 이상': 0, '시니어': 0,
    }
    ...
    sorted_careers = sorted(careers_dict.items(), key=lambda item: item[1], reverse=True)
    ...
    sorted_developers = sorted(developers_dict.items(), key=lambda item: item[1], reverse=True)
    ...
    sorted_d_langs = sorted(d_langs_dict.items(), key=lambda item: item[1], reverse=True)
    ...
    sorted_u_langs = sorted(u_langs_dict.items(), key=lambda item: item[1], reverse=True)
    print(sorted_u_langs)

    # 6. 코딩을 배운 방법
    methods = Survey.objects.all().values_list('how_to_learn', flat=True)
    methods_dict = {
        '학교': 0, '책': 0,  '온라인 강의': 0, '코딩 부트캠프': 0, 
        '기타 자료 (동영상, 블로그 등)': 0, '기타': 0,  
    }
    ...
    sorted_methods = sorted(methods_dict.items(), key=lambda item: item[1], reverse=True)
    print(sorted_methods)

    # 7. 하루 평균 학습 시간
    hours = Survey.objects.all().values_list('daily_learning_hours', flat=True)
    hours_dict = {
        '1시간 미만': 0, '1시간 이상 3시간 미만': 0,
        '3시간 이상 5시간 미만': 0, '5시간 이상 10시간 미만': 0,
        '10시간 이상': 0,
    }
    ...
    # 8. 학력
    educations = Survey.objects.all().values_list('education', flat=True)
    educations_dict = {
        '중졸 이하': 0, '고졸': 0, '전문대졸': 0, '대졸 이상': 0,
    }
    
    for education in educations:
        educations_dict[education] += 1
    
    for k, v in educations_dict.items():
        educations_dict[k] = int(round(v / total_people_cnt, 2) * 100)

    sorted_educations = sorted(educations_dict.items(), key=lambda item: item[1], reverse=True)
    print(sorted_educations)

    # 9. 학위
    degrees = Survey.objects.all().values_list('degree', flat=True)
    degrees_dict = {
        '없음': 0, '전문학사': 0, '학사': 0, '석사': 0, '박사': 0, 
    }
    ...
    sorted_degrees = sorted(degrees_dict.items(), key=lambda item: item[1], reverse=True)
    ...
    sorted_genders = sorted(genders_dict.items(), key=lambda item: item[1], reverse=True)
    ...
    context = {
        ...
        'sorted_educations': sorted_educations,
